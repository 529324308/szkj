...
`CesiumMap.vue` 是项目的主地图壳组件，承载整个平台的顶层导航、Cesium 地球初始化、测绘工具、数治地价、智能分析、数据管理、管理中心以及底部地图辅助信息展示。它通过 `useCesium(containerId)` 统一管理 Cesium Viewer、图层、事件监听、地形、相机与基础交互，并把各业务模块以子组件形式挂载在同一个地图工作台上。
