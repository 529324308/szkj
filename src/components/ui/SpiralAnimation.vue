<template>
  <div class="spiral-animation">
    <canvas ref="canvasRef" class="spiral-animation__canvas"></canvas>
  </div>
</template>
...
class AnimationController {
  ...
  setProgress(progress) {
    this.progress = this.constrain(Number(progress) / 100, 0, 1);
  }
  ...
  spiralPath(p) {
    ...
    const r = 170 * this.fieldScale * Math.sqrt(p);
  ...
  render() {
    ...
    this.displayProgress = this.lerp(this.displayProgress, this.progress, 0.12);
    const t1 = this.constrain(this.map(this.time, 0, this.changeEventTime + 0.25, 0, 1), 0, 1);
    const rotationProgress = this.ease(this.displayProgress, 2.7);
  ...
  drawTrail(t1) {
    for (let i = 0; i < this.trailLength; i += 1) {
      ...
      const offset = new Vector2D(position.x + 5 * this.fieldScale, position.y + 5 * this.fieldScale);
...
class Star {
  constructor(cameraZ, cameraTravelDistance) {
    ...
    this.distance = (30 * Math.random() + 15) * Star.fieldScale;
...
function createAnimation() {
  ...
  animation = new AnimationController(canvas, ctx, dpr, size);
  animation.setProgress(props.progress);
  animation.render();
}
