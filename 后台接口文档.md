...
| 8 | 通知接口 | 实时通知、提醒事件、通知列表 |
| 9 | C# 后端建议模型 | Controller、实体、数据库关联表、初始化数据、权限标识 |
| 10 | 联调检查清单 | 页面接口覆盖和完整性复查 |

## 接口数量统计

统计口径：按文档中已定义的 HTTP 接口统计；同一路径不同方法分别计为不同接口；包含“可选”的日报草稿接口、实时连接凭证接口和通知中心预留接口。当前合计 **38 个接口**。
...
| 通知接口 | 5 | 实时连接凭证、通知列表、未读数量、单条已读、全部已读 |
| 合计 | 38 | 管理中心全部已设计接口 |
...
| 34 | 通知接口 | 获取实时连接凭证 | GET | `/api/PersonalCenter/realtime/token` |
| 35 | 通知接口 | 获取通知列表，可选 | GET | `/api/PersonalCenter/notifications` |
| 36 | 通知接口 | 获取未读通知数量，可选 | GET | `/api/PersonalCenter/notifications/unread-count` |
| 37 | 通知接口 | 标记单条通知已读，可选 | POST | `/api/PersonalCenter/notifications/{notificationId}/read` |
| 38 | 通知接口 | 全部通知标记已读，可选 | POST | `/api/PersonalCenter/notifications/read-all` |
...
| WebSocket | 先通过 HTTP 获取短期 `wsToken`，再用于 `wss://` 握手，不建议在 URL 中直接传长期 `accessToken` |
...
| report_submit_reminder | 日报提交提醒 | 定时任务提醒员工提交日报 |
| project_progress_reminder | 项目进度提醒 | 定时任务提醒员工更新进度 |
| project_due_soon | 项目即将到期提醒 | 定时任务提醒项目即将到期 |
| system_announcement | 系统公告通知 | 后续通知中心统一公告 |
...
| avatar | 头像业务 | 个人设置头像 |
| system | 系统业务 | 系统公告、全局提醒、无具体业务对象通知 |
...
| title | string | 是 | 标题 |
| content | string | 是 | 内容 |
| type | string | 是 | 通知类型；当前最少支持项目通知、日报通知、提醒通知和系统公告，完整枚举见 8.6 |
| category | string | 否 | 分类：`project`、`report`、`reminder`、`system`，后续通知页建议使用 |
| priority | string | 否 | 优先级：`low`、`medium`、`high`、`critical` |
| bizType | string | 是 | 业务类型：`project` 项目；`projectProgress` 项目进度；`report` 日报；`system` 系统消息 |
| bizId | string | 否 | 业务 ID |
| bizDataId | string | 否 | 业务对象 ID，如项目 ID、日报 ID |
| route | string | 否 | 前端跳转地址 |
| routeParams | object | 否 | 前端跳转参数 |
| actionText | string | 否 | 操作文案 |
| isRead | bool | 是 | 是否已读 |
...
> 本节补充 WebSocket 实时方案，并把后续“通知中心页面”所需的数据结构一次性预留。  
> 约定：业务提交成功后，先写业务表，再写 `Notification` 表，最后推 WebSocket 事件；前端实时刷新依赖 WebSocket，离线补偿和列表查询依赖 HTTP。

后端职责：

| 职责 | 说明 |
|---|---|
| 事件生产 | 在项目下发、项目进度提交、日报提交、日报批注、项目审核通过/驳回成功后，统一生成通知 |
| 定时提醒 | 通过后台定时任务生成“日报未提交提醒”“项目进度提醒”“项目即将到期提醒” |
| 实时推送 | 将新通知广播给当前接收人的所有在线连接 |
| 离线补偿 | 用户不在线时只落库不丢失，下次进入通过通知列表和未读数接口补齐 |
| 权限控制 | 主管只收到本部门相关消息，员工只收到本人相关消息，管理员可收到全局审核/统计类消息 |
| 多端同步 | 单条已读和全部已读后，要向该用户其他在线端同步已读状态 |

推荐处理顺序：

1. 业务事务提交成功。
2. 根据业务类型计算接收人集合。
3. 为每个接收人分别写一条 `Notification` 记录。
4. 组装 `RealtimeEnvelopeDto` 推送给在线连接。
5. 推送失败不回滚主业务，客户端后续通过 HTTP 刷新兜底。

### 8.4 建立实时连接

| 项 | 内容 |
|---|---|
| 页面 | 管理中心首页、项目页、日报页；后续通知中心 |
| 功能 | 建立当前用户实时连接，接收通知与提醒事件 |
| 协议 | WebSocket |
| 地址 | `wss://{host}/ws/PersonalCenter?token={wsToken}&lastEventId={eventId}` |
| 权限 | admin / manager / employee |
...
| 规则 | 说明 |
|---|---|
| 短令牌 | 前端必须先调用 `GET /api/PersonalCenter/realtime/token` 获取短期 `wsToken` |
| 令牌时效 | 建议 `wsToken` 有效期 `60-120` 秒，只用于握手，不用于普通 HTTP |
| 心跳 | 服务端每 `30` 秒发 `ping`，客户端收到后立即回复 `pong` |
| 断线重连 | 客户端重连时应重新申请 `wsToken`，并带上最近一次处理成功的 `lastEventId` |
| 事件补发 | 服务端建议保留最近 `15` 分钟或最近 `1000` 条事件用于补发 |
| 全量刷新 | 如果 `lastEventId` 不存在或补发窗口已过，服务端返回 `needRefresh=true`，客户端改走 HTTP 拉全量 |
| 去重 | 客户端按 `eventId` 去重；服务端同一通知只生成一次事件，但允许同一用户多端同时收到 |
...
| eventType | string | 是 | 实时事件类型，见 8.6 |
| occurredAt | datetime | 是 | 事件发生时间 |
| notificationId | string | 否 | 对应通知 ID，心跳事件可为空 |
| unreadCount | int | 否 | 当前用户最新未读数 |
| needRefresh | bool | 否 | 是否要求客户端做 HTTP 全量刷新 |
| refreshHints | array<RefreshHintDto> | 否 | 刷新提示 |
| notification | NotificationListItemDto | 否 | 通知详情，便于前端直接插入列表 |
| payload | object | 否 | 附加业务字段 |

`RefreshHintDto`：

| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| target | string | 是 | 刷新目标：`overview`、`projects`、`projectDetail`、`reports`、`reportDetail`、`notifications` |
| bizId | string | 否 | 相关业务 ID |
...
|---|---|
| 页面 | 管理中心全部页面 |
| 功能 | 获取 WebSocket 连接短期凭证 |
| 方法 | GET |
| 地址 | `/api/PersonalCenter/realtime/token` |
| 权限 | admin / manager / employee |

请求参数：无。

返回参数 `RealtimeTokenDto`：

| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| wsToken | string | 是 | WebSocket 握手短期令牌 |
| expiresAt | datetime | 是 | 凭证过期时间 |
| wsUrl | string | 是 | WebSocket 地址前缀 |
| heartbeatIntervalSeconds | int | 是 | 心跳间隔秒数 |
| replayWindowSeconds | int | 是 | 断线补发时间窗口 |

### 8.6 实时事件与通知类型补充

业务通知事件：

| 事件类型 | 触发时机 | 默认接收人 | 是否写入通知列表 | 说明 |
|---|---|---|---|---|
| `project_assigned` | 项目下发成功 | 执行人 | 是 | 项目创建后即时通知 |
| `project_progress_updated` | 员工提交项目进度成功 | 所属部门主管；管理员可选 | 是 | 同步首页任务进度流 |
| `report_submitted` | 员工/主管提交日报成功 | 所属部门主管 | 是 | 提醒主管处理日报 |
| `report_commented` | 主管批注日报成功 | 日报提交人 | 是 | 提醒员工查看批注 |
| `project_approved` | 管理员审核通过成功 | 项目执行人 | 是 | 同步项目完成状态 |
| `project_rejected` | 管理员驳回成功 | 项目执行人 | 是 | 提醒继续补充 |

提醒类事件：

| 事件类型 | 触发来源 | 默认接收人 | 是否写入通知列表 | 说明 |
|---|---|---|---|---|
| `report_submit_reminder` | 定时任务 | 当天尚未提交日报的员工 | 是 | 支持多时段催办 |
| `project_progress_reminder` | 定时任务 | 项目执行人 | 是 | 如连续 N 天无进度更新 |
| `project_due_soon` | 定时任务 | 项目执行人、负责人、管理员可选 | 是 | 如剩余 3 天、1 天、已逾期 |
| `system_announcement` | 后续管理后台/脚本 | 指定用户、角色或全员 | 是 | 为后续通知中心系统公告预留 |
...
| `ping` | 服务端心跳 | 否 |
| `pong` | 客户端心跳响应 | 否 |
| `notification_read_sync` | 多端同步已读状态 | 否 |

### 8.7 定时提醒建议

| 任务名 | 执行时机 | 规则 |
|---|---|---|
| 日报未提交扫描 | 每工作日 `17:00`、`18:30` | 扫描在职/试用员工中当天仍未提交日报的人，生成 `report_submit_reminder` |
| 项目即将到期扫描 | 每小时或每天 `09:00` | 扫描截止日剩余 `3` 天、`1` 天和已逾期项目，生成 `project_due_soon` |
| 项目长时间未更新扫描 | 每天 `09:30` | 扫描最近 `N` 天没有 `ProjectProgressSubmission` 的进行中项目，生成 `project_progress_reminder` |
| 待审核超时扫描 | 每小时 | 扫描“待审核”超过阈值的项目，提醒管理员尽快处理 |

### 8.8 通知列表字段扩展

> 为后续独立通知页预留字段，建议把当前 `NotificationDto` 升级为 `NotificationListItemDto`。
...
| title | string | 是 | 标题 |
| content | string | 是 | 内容 |
| type | string | 是 | 通知类型，见 8.6 |
| category | string | 是 | 分类：`project`、`report`、`reminder`、`system` |
| priority | string | 是 | 优先级：`low`、`medium`、`high`、`critical` |
| bizType | string | 是 | 业务类型：`project`、`projectProgress`、`report`、`system` |
| bizId | string | 否 | 通知主业务 ID，系统公告可为空 |
| bizDataId | string | 否 | 业务对象 ID，如项目 ID、日报 ID |
| route | string | 否 | 前端跳转地址 |
| routeParams | object | 否 | 前端跳转参数，如 `{ "projectId": "proj-001" }` |
| actionText | string | 否 | 操作文案，如“查看项目”“查看日报” |
| isRead | bool | 是 | 是否已读 |
| readAt | datetime | 否 | 已读时间 |
| senderName | string | 否 | 触发人姓名，系统任务可为空 |
| createdAt | datetime | 是 | 创建时间 |

列表接口建议额外返回 `NotificationStatsDto`：

| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| unreadTotal | int | 是 | 未读总数 |
| unreadProjectCount | int | 否 | 项目类未读数 |
| unreadReportCount | int | 否 | 日报类未读数 |
| unreadReminderCount | int | 否 | 提醒类未读数 |
| unreadSystemCount | int | 否 | 系统类未读数 |
...
| 字段 | 类型 | 必填 | 默认值 | 可选值 | 中文说明 |
|---|---|---|---|---|---|
| type | string | 否 | 空 | 见 8.6 | 按通知类型筛选 |
| category | string | 否 | 空 | `project`、`report`、`reminder`、`system` | 按分类筛选 |
| priority | string | 否 | 空 | `low`、`medium`、`high`、`critical` | 按优先级筛选 |
| startDate | date | 否 | 空 | 日期 | 创建时间开始 |
| endDate | date | 否 | 空 | 日期 | 创建时间结束 |
| keyword | string | 否 | 空 | 任意关键词 | 标题/内容搜索 |
...
| 功能 | 获取当前用户未读通知统计 |
| 方法 | GET |
| 地址 | `/api/PersonalCenter/notifications/unread-count` |
| 权限 | admin / manager / employee |

请求参数：无。

返回参数：`NotificationStatsDto`。
...
| 功能 | 把指定通知标记为已读 |
| 方法 | POST |
| 地址 | `/api/PersonalCenter/notifications/{notificationId}/read` |
| 权限 | admin / manager / employee |
...
| 数据归属 | 只能标记当前用户自己的通知 |
| 幂等性 | 已读通知重复调用仍返回成功 |
| 多端同步 | 成功后向当前用户其他在线端推送 `notification_read_sync` |
...
|---|---|
| 页面 | 后续通知中心 |
| 功能 | 当前筛选范围或全部未读通知一键标记已读 |
| 方法 | POST |
| 地址 | `/api/PersonalCenter/notifications/read-all` |
| 权限 | admin / manager / employee |
...
| 字段 | 类型 | 必填 | 默认值 | 可选值 | 中文说明 |
|---|---|---|---|---|---|
| type | string | 否 | 空 | 见 8.6 | 只标记某类型通知 |
| category | string | 否 | 空 | `project`、`report`、`reminder`、`system` | 只标记某分类通知 |
| beforeDate | datetime | 否 | 空 | 日期时间 | 只标记某时间之前的通知 |

返回参数：

| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| affectedCount | int | 是 | 本次标记已读的数量 |
| unreadCount | int | 是 | 标记后剩余未读数 |
...
| `RealtimeController` 或 `PersonalCenterRealtimeController` | WebSocket 短期凭证、实时连接配置 |
| `NotificationsController` 或 `PersonalCenterNotificationsController` | 通知列表、未读数、单条已读、全部已读 |
...
| `Notification` | `Id`、`ReceiverId`、`Title`、`Content`、`Type`、`Category`、`Priority`、`BizType`、`BizId`、`BizDataId`、`Route`、`RouteParamsJson`、`ActionText`、`SenderId`、`IsRead`、`ReadAt`、`CreatedAt` |
...
| 项 | 说明 |
|---|---|
| `RealtimeToken` | 可不落库，建议用 JWT 或服务端缓存保存短期凭证 |
| 最近事件缓存 | 建议用 Redis Stream、List 或内存队列保留最近一段时间事件，用于 `lastEventId` 补发 |
...
| SenderId | uniqueidentifier | FK -> Employee.Id | 是 | null | 触发人 ID；定时任务或系统公告可为空 |
| Title | nvarchar(100) |  | 否 | 无 | 通知标题 |
| Content | nvarchar(500) |  | 否 | 无 | 通知内容 |
| Type | nvarchar(50) |  | 否 | 无 | 通知类型 |
| Category | nvarchar(20) |  | 否 | reminder | 分类：project、report、reminder、system |
| Priority | nvarchar(20) |  | 否 | medium | 优先级：low、medium、high、critical |
| BizType | nvarchar(50) |  | 否 | 无 | 业务类型 |
| BizId | uniqueidentifier |  | 是 | null | 通知主业务 ID，如进度提交 ID、日报 ID |
| BizDataId | uniqueidentifier |  | 是 | null | 业务对象 ID，如项目 ID、日报 ID |
| Route | nvarchar(300) |  | 是 | null | 前端跳转地址 |
| RouteParamsJson | nvarchar(max) |  | 否 | `{}` | 跳转参数 JSON |
| ActionText | nvarchar(50) |  | 是 | null | 操作文案，如“查看项目” |
| ReminderRuleKey | nvarchar(50) |  | 是 | null | 提醒规则标识，如 report_daily_1700 |
| IsRead | bit |  | 否 | 0 | 是否已读 |
| ReadAt | datetime2 |  | 是 | null | 阅读时间 |
| DeliveredAt | datetime2 |  | 是 | null | 最近一次成功推送到在线端的时间，可选 |
| CreatedAt | datetime2 |  | 否 | getdate | 创建时间 |
...
| IX_Notification_Receiver_CreatedAt | ReceiverId + CreatedAt | 普通索引 | 当前用户通知按时间倒序 |
| IX_Notification_Receiver_Category | ReceiverId + Category + IsRead | 普通索引 | 通知中心按分类筛选 |
| IX_Notification_Receiver_Type | ReceiverId + Type + IsRead | 普通索引 | 通知中心按类型筛选 |
| IX_Notification_Biz | BizType + BizId | 普通索引 | 按主业务追踪通知 |
| IX_Notification_BizData | BizType + BizDataId | 普通索引 | 按业务对象追踪通知 |
| IX_Notification_ReminderRule | ReceiverId + ReminderRuleKey + CreatedAt | 普通索引 | 防止同一提醒规则短时间重复轰炸 |
...
| 通知归属隔离 | C# 业务校验 | 用户只能查询和修改自己的通知已读状态 |
| 提醒去重 | C# + 定时任务幂等校验 | 同一规则同一时间窗口内避免重复生成同类提醒 |
| 项目进度合法 | C# 业务校验 | 数据库只存结果，节点流转由服务层校验 |
| 项目审核合法 | C# 业务校验 | 仅管理员，且项目必须待审核、100%、回款完成 |
| 主管数据范围 | C# 查询过滤 | 主管只能访问本部门员工、项目、日报 |
| 通知后置发送 | 事务提交后事件总线/后台任务 | 先提交业务与通知记录，再做实时推送，避免推送成功但事务回滚 |
...
| `notification:view` | 查看通知列表与未读数 | admin / manager / employee |
| `notification:read` | 标记通知已读 | admin / manager / employee |
| `realtime:connect` | 获取实时连接凭证与建立实时连接 | admin / manager / employee |
