...
| visibleSections | array\<string\> | 是 | 管理中心可见页面：`overview` 概览首页；`employees` 员工管理；`projects` 项目管理；`reports` 日报管理；`settings` 个人设置 |
...
| 序号 | 页面/模块 | 功能 | 方法 | 地址 |
|---:|---|---|---|---|
| 1 | 公共接口 | 获取当前登录用户与权限 | GET | `/api/PersonalCenter/current-user` |                                                 （测试通过） 
| 2 | 公共接口 |   | GET | `/api/PersonalCenter/options` |                                                                         （测试通过）
| 3 | 公共接口 | 上传附件 | POST | `/api/PersonalCenter/files` |                                              （测试未通过）
| 4 | 公共接口 | 下载附件 | GET | `/api/PersonalCenter/files/{fileId}/download` |                             （测试未通过）

| 5 | 管理中心概览 | 获取管理中心概览数据 | GET | `/api/PersonalCenter/overview` |                                                   （测试通过，需增加一个卡片数据）
| 6 | 管理中心概览 | 获取重点关注状态 | GET | `/api/PersonalCenter/focus-state` |                       （等待测试）
| 7 | 管理中心概览 | 更新重点关注状态 | PUT | `/api/PersonalCenter/focus-state` |                       （等待测试）

| 8 | 员工管理页 | 获取部门列表 | GET | `/api/PersonalCenter/departments` |                                                         （测试通过）
| 9 | 员工管理页 | 新增部门 | POST | `/api/PersonalCenter/departments` |                                                            （测试通过）    
| 10 | 员工管理页 | 员工列表 | GET | `/api/PersonalCenter/employees` |                                                              （测试通过）     
| 11 | 员工管理页 | 新增员工 | POST | `/api/PersonalCenter/employees` |                                                             （测试通过） 
| 12 | 员工管理页 | 编辑员工 | PUT | `/api/PersonalCenter/employees/{employeeId}` |                                                 （测试通过） 
| 13 | 员工管理页 | 删除员工 | DELETE | `/api/PersonalCenter/employees/{employeeId}` |                                              （测试通过）
| 14 | 员工管理页 | 根据部门获取可选执行人 | GET | `/api/PersonalCenter/departments/{departmentId}/executors` |                      （测试通过）

| 15 | 项目管理页 | 项目列表 | GET | `/api/PersonalCenter/projects` |                                                               （测试通过）
| 16 | 项目管理页 | 项目详情 | GET | `/api/PersonalCenter/projects/{projectId}` |                                                   （测试通过）
| 17 | 项目管理页 | 下发项目 | POST | `/api/PersonalCenter/projects` |                                                              （测试通过）
| 18 | 项目管理页 | 提交项目进度 | POST | `/api/PersonalCenter/projects/{projectId}/progress` |                                     （测试通过）
| 19 | 项目管理页 | 管理员审核通过项目 | POST | `/api/PersonalCenter/projects/{projectId}/approve` |                                 （测试通过）
| 20 | 项目管理页 | 管理员驳回项目 | POST | `/api/PersonalCenter/projects/{projectId}/reject` |                                     （测试通过）
| 21 | 项目管理页 | 下载项目全部附件 | GET | `/api/PersonalCenter/projects/{projectId}/attachments/download-all` | （等待测试）
